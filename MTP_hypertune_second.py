...
if __name__=='__main__':
    ...
    PATH = '/home/frank/document/Python/Factors/output/hypertune/level1'
    ...
    for name in metrics.index[250:350]:
    # for name in ['Close.Pct_Change','Open/Close.SignedPower.Abs.Kurt','Close.Rank']:
        ...
        # name = 'BuyerRatio.SignedPower.Skew.Kurt'
        # name = 'Open/Low.Quantile25.Skew.Skew'
        # name = 'High/Open.Max.Quantile50.Pct_Change'
        ...
        # if os.path.exists(f"{PATH}/{ix}/categories_{ix}.jpg"):
        #     continue

        if not os.path.isdir(f'{PATH}/{ix}'):
            os.mkdir(f'{PATH}/{ix}')
        ...
        FNAME = f'{PATH}/{ix}/hypertune_First_{ix}.csv'
        ...
        first_result = first_result[(first_result['FitnessValue']>0) & (first_result['FitnessValue']<first_result.FitnessValue.drop_duplicates().quantile(0.99))]
        first_result['FitnessValue'] = first_result['FitnessValue'].round(1)
        first_result = first_result.sort_values(['Sharpe Decay','FitnessValue'],ascending = [True,False])#.drop_duplicates('Sharpe')
        first_result = first_result.iloc[:int(first_result.shape[1] * 0.5)]
        ...
        vol_len = first_result['vol_len'].unique()
        param_count = sum([1 for operator in operators if operator not in FORMULAS_PARAM_FREE])
        params = []
        for i in range(param_count):
            param = first_result[first_result.columns[i+1]]
            start = int(np.floor(first_result[first_result.columns[1]].quantile(0.25) / 8) * 8)
            end = int(np.floor(first_result[first_result.columns[1]].quantile(0.95) / 8) * 8)
        ...
        ix = strategies.index(name.replace('_Reverse',''))
        FNAME = f'{PATH}/{ix}/hypertune_Second_{ix}.csv'
        keys = 'Ann. Return(%),MDD(%),Ret/Mdd,Ann. Sharpe,ATH_Score,Monthly_Return_Score,Cummax_Score,FitnessValue,Sharpe Decay'.split(',')
        with open(FNAME,'w') as f:
            f.write(','.join(['vol_len'] + [f'len{i+1}'for i in range(sum([1 for operator in operators if operator not in FORMULAS_PARAM_FREE]))])+',Ann. Return(%),MDD(%),Ret/Mdd,Ann. Sharpe,ATH_Score,Monthly_Return_Score,Cummax_Score,FitnessValue,Sharpe Decay\n')
        ...
        second_result = second_result.sort_values(['FitnessValue','Sharpe Decay'],ascending = [False,True])
        ...
        selected.to_csv(f'{PATH}/{ix}/Ensamble_{ix}.csv')
        ...
        result.to_csv(f"{PATH}/{ix}/{ix}_Ensamble_nav.csv")
        plt.savefig(f"{PATH}/{ix}/{ix}.jpg",dpi = 500)
        plt.close()        
        ...
        plt.savefig(f"{PATH}/{ix}/selection_result_{ix}.jpg",dpi = 500)
        plt.close()     
        ...
        plt.savefig(f"{PATH}/{ix}/categories_{ix}.jpg",dpi = 500)
        plt.close()        
        print(ix)
        # break
