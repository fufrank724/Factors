...
if __name__=='__main__':
    ...
    strategies = pd.read_csv('/home/frank/document/Python/Factors/output/backtest_result/D/level1_IC_metrics.csv').Strategy.sort_values().to_list()
    ...
    # metrics['Sharpe_Decay'] = (metrics['Outsample Sharpe']/metrics['Insample Sharpe'] - 1).abs().round(1)
    ...
    # metrics[['Sharpe','Insample Sharpe','Outsample Sharpe']] = metrics[['Sharpe','Insample Sharpe','Outsample Sharpe']].abs().round()
    ...
    PATH = '/home/frank/document/Python/Factors/output/hypertune_individual/level1'
    if not os.path.isdir(f'{PATH}/'):
        os.mkdir(f'{PATH}/')

    # for name in metrics.index[:100]:
    for name in ['Open/Close.SignedPower.Abs.Kurt']:
